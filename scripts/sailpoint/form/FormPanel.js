...
Ext.define('SailPoint.form.FormPanel', {
    ...
    constructor: function(config) {
        ...
        if (config.wizard === true) {
            ...
            if (config.buttons){
                Ext.each(config.buttons, function(button){
    ...
    afterRender : function(){
        ...
        if (field){
            ...
            if(tdDir && tdDir[0] && tdDir[0].value !== "") {
                ...
                inputs = Ext.DomQuery.select('input', Ext.getDom(fieldset));
    ...
    prepareItemConfig : function(item){
        ...
        if (item.xtype === 'itemselector'){
            ...
            // set default height because IE10+ collapses otherwise
            item.height = item.height || 300;
            Ext.define(modelId, {
            ...
            if (item.allowedValues) {
                Ext.each(item.allowedValues, function(item){
    ...
    handleEventListeners : function(item){
        if (item){
                ...
                if (isText === true){
                    ...
                    item.on('blur', function(field, blurEvent, eOpts){
                        if (field.getValue() !== field.valueBeforeFocus){
                            //in case the field needs coercion to string to do a proper comparison
                            if(field.getValue() !== null && field.valueBeforeFocus !== null) {
                                if (field.getValue().toString() !== field.valueBeforeFocus.toString()){
                                    Page.fireEvent('refresh', field);
                                }
                            } else {
                                Page.fireEvent('refresh', field);
            ...
            if (item.xtype === 'textfield' || item.xtype === 'secret' || item.xtype === 'date' || item.xtype === 'spdate') {
    ...
    eSigCallback : function(name, pass) {
        if(name !== this.nAuthId && name !== this.oAuthId) {
            Ext.getDom(this.containerFormName+':signatureAuthId').value = name;
        }
        Ext.getDom(this.containerFormName+':signaturePass').value = pass;
        this.getSubmitButton().click();
    }
