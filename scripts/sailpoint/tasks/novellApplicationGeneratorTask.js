...
SailPoint.Tasks.Novell.ApplicationGenerator = function()
{
	...
	{
		return Ext.getDom('applicationName').value;
	}
	...
	var _initializeAppList = function()
	{
		...
		var initialValues = Ext.getDom('novellAppSuggestVal').value;
		...
		{
			_addItems(Ext.Array.toArray(initialValues.split(',')));
		}
	...
	return {
		init : function() 
		{
	        ...
	        _sailpointAppSuggest.on('select', function(suggestField, record, index) {
                Ext.getDom('applicationName').value = record[0].get('displayName');
	        });
		    
		    
		    _fetchAppsButton = new Ext.Button({
		    	renderTo: 'fetchAppsButtonDiv',
		    	text: '#{msgs.nidm_fetch_novell_applications}',
