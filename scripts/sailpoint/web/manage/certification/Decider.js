...
Ext.define('SailPoint.Decider', {
	...
	unmaskGrids : function() {
        Ext.each(this.gridIds, function(gridId){
    ...
    */
    removeAll: function(){
        ...
        // of a decision
        Ext.each(Ext.query('.delegationReview'), function(item){
            Ext.fly(item).show();
        });
    },
    ...
    */
    ...
    updateStatusWidget : function(respObj) {
      ...
      if (Ext.getDom('editForm:rerenderCertificationSummaryBtn'))
          Ext.getDom('editForm:rerenderCertificationSummaryBtn').click();
    ...
    getGridFilter : function(){
        ...
        if (Ext.getDom('certificationFilterString') && Ext.getDom('certificationFilterString').value != ""){
            return Ext.getDom('certificationFilterString').value;
        }
        return null;
    },
    ...
    generateEntityDecision : function(actionType, sourceGrid){
        ...
        if (actionType == SailPoint.Decision.STATUS_UNDO || actionType == SailPoint.Decision.STATUS_REASSIGN){
            ...
            if (decision.selectionCriteria.selectAll &&
                ...
                Ext.each(sourceGrid.getSelectionModel().getSelection(), function(record){
            ...
            success: function(response){
                try{
                    ...
                    if (response.success){
                        ...
                        Ext.each(this.decider.gridIds, function(gridId){
                            var grid = Ext.getCmp(gridId);
                            grid.unmask();
