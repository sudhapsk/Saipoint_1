...
function checkAllItemsSelected() {
    ...
    if( usingBulkDecisions && !usingBulkDecisions.disableDecisionColumn) {
		keys = Ext.Object.getKeys( SailPoint.RemoteDecisions );
		count = keys.length;
		for( i = 0; i < count; i++ ) {
			id = keys[ i ] 
...
function showFinishAllMessage() {
    Ext.getDom('finishAllItemsDiv').style.display = '';
}
...
SailPoint.web.workitem.addApprovalItemComment = function(approvalItemId, btnName) {
    SailPoint.showAddCommentDlg(function(btn, text){
        if (btn == 'ok'){
            var hiddenVal = Ext.getDom('editForm:approvalComment'),
                hiddenId = Ext.getDom('editForm:approvalId');
            if(hiddenVal) {
...
function saveActivationDates() {
    ...
    var sunriseVal = Ext.get('editForm:approvalSunrise');
    if(sunriseVal) {
        if(sunrise.getValue()) {
            var sunval = sunrise.getValue();
            sunval.setHours(me.form.sunriseHour);
            sunval.setMinutes(me.form.sunriseMin);
            sunriseVal.dom.value = sunval.getTime();
        }
        else {
            sunriseVal.dom.value = "";
        }
    }
    var sunsetVal = Ext.get('editForm:approvalSunset');
    if(sunsetVal) {
        if(sunset.getValue()) {
            var setval = sunset.getValue();
            setval.setHours(me.form.sunsetHour);
            setval.setMinutes(me.form.sunsetMin);
            sunsetVal.dom.value = setval.getTime();
        } else {
            sunsetVal.dom.value = "";
        }
        
    }
    var hiddenId = Ext.getDom('editForm:approvalId');
    
        hiddenId.value=me.roleId;
        
        Ext.getDom('editForm:updateActivationDates').click();
...
function setButtonsDisabled(newDisabledVal){
    ...
    var completeButton = Ext.getDom('editForm:completeStub');
    var saveButton = Ext.getDom('editForm:saveStub');
