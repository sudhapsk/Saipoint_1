        ...
        toolbar = [ {
            ...
            listeners: {
                'select': function( combo, records, ops ) {
                      var xtraParms = {},
                          count = Ext.Object.getKeys( SailPoint.DecisionTracker ).length,
                          filterParam,
                          proxy,
                          newDecisionTracker = {};
        ...
        bulkDecisionSelect.on('select', function(combo, record, index){
            ...
            for( i = 0; i != numberOfSelectedIds; i++ ) {
                ...
                if( !Ext.Array.contains( excludedIds, selectedId ) ) {
                  if( Ext.getDom( 'decision_' + selectedId ) ) {
                      if( decision === 'Finished' ) {
                          Ext.getDom('editForm:secretApproveButton_' + selectedId).click();
                      } else {
                          Ext.getDom('reject-gen' + selectedId).click();
                      }
                  }
...
SailPoint.workitem.ApprovalItemGrid.deleteButtonClick = function(id) {
    ...
    var container = Ext.getDom('decision_' + id);
    if (container != null) { 
    ...
    SailPoint.DecisionTracker[id] = ImageRadio.getRadio(Ext.getDom('delete-gen' + id)).value;
    SailPoint.web.workitem.toggleItemState(id, SailPoint.DecisionTracker[id]);
    
    if (null == ImageRadio.getRadioValue(Ext.getDom('delete-gen' + id))) {
...
SailPoint.workitem.ApprovalItemGrid.renderWorkItemDecisionColumn = function( value, metadata, record, rowIdx, colIdx, store ) {
  ...
  html +=             '<input type="hidden" name="editForm:secretApproveButton_' + record.data.id + '" id="editForm:secretApproveButton_' + record.data.id + '" onclick="if(!'+decisionDisabled+') { if (ImageRadio.radioClickCanceled) { ImageRadio.radioClickCanceled = false; return false; } SailPoint.DecisionTracker[ \'' + record.data.id+ '\' ] = ImageRadio.getRadio(Ext.getDom(\'approve-gen' + record.data.id + '\')).value; SailPoint.web.workitem.handleItemStateChange(\'' + record.data.id + '\', SailPoint.DecisionTracker[ \'' + record.data.id+ '\' ]); if( SailPoint.BulkDecision.overridden ) { SailPoint.BulkDecision.overridden.push( \'' + record.data.id + '\' );}}"/>';
  html +=           '</div>';
  html +=         '</td>';
  html +=         '<td style="padding: 0px; border: 0px; " width="23px">';
  ...
  html +=             '<input type="hidden" name="editForm:secretRejectButton_' + record.data.id + '" id="editForm:secretRejectButton_' + record.data.id + '" onclick="if(!'+decisionDisabled+') { if (ImageRadio.radioClickCanceled) { ImageRadio.radioClickCanceled = false; return false; } SailPoint.DecisionTracker[ \'' + record.data.id+ '\' ] = ImageRadio.getRadio(Ext.getDom(\'reject-gen' + record.data.id + '\')).value; SailPoint.web.workitem.handleItemStateChange(\'' + record.data.id + '\', SailPoint.DecisionTracker[ \'' + record.data.id+ '\' ]); if( SailPoint.BulkDecision.overridden ) { SailPoint.BulkDecision.overridden.push( \'' + record.data.id + '\' );}}"/>';
  html +=           '</div>';
  html +=         '</td>';
  html +=       '</tr>';
  html +=     '</tbody>';
