...
SailPoint.LCM.ManageAccounts.refreshButtonClicked = function(radio) {
  ...
  Ext.get('status_loading_'+id).show();
  
  /** Submit targeted aggregation request **/
  Ext.getDom('editForm:linkId').value = id;
  Ext.getDom('editForm:refreshLinkStatusBtn').click();
};
...
SailPoint.LCM.ManageAccounts.refreshButtonComplete = function(radio) {
  var summaryString = Ext.getDom('editForm:refreshSummaryStatus').innerHTML;
...
SailPoint.LCM.ManageAccounts.buttonRenderer = function(value, p, r) {
  ...
  var callbacks = {
      ...
      selected: function(label, value, record) {
          ...
          if (unsavedChanges) {
              var operation = null;
              Ext.Object.each(unsavedChanges, function(key, val) {
                  if (key === record.getId()) {
                      operation = val;
...
SailPoint.LCM.ManageAccounts.isLocked = function(id) {
    ...
    Ext.each(SailPoint.LCM.ManageAccounts.currentRequests, function(req) {
...
SailPoint.LCM.ManageAccounts.submit = function() {
  ...
  if (changes) {
    Ext.Object.each(changes, function(key, val) {
      var id = key;
      var op = val;
...
SailPoint.LCM.ManageAccounts.autoRefreshCall = function(id){
    ...
    /** Hide the radio button **/
    Ext.getDom('lcmRefreshRadio_'+id).style.display = 'none';
    ...
    Ext.Ajax.request({
         ...
         failure: function (response) {          
          Ext.get('status_complete_'+id).show();
          Ext.get('status_loading_'+id).setVisibilityMode(Ext.Element.DISPLAY).hide();
          Ext.get('lcmRefreshRadio_'+id).setVisibilityMode(Ext.Element.DISPLAY).hide();
...
SailPoint.LCM.ManageAccounts.processRefreshSummary = function(refreshSummary){    
      ...
      Ext.get('status_complete_'+refreshSummary.id).show();
      Ext.get('status_loading_'+refreshSummary.id).setVisibilityMode(Ext.Element.DISPLAY).hide();
      Ext.get('lcmRefreshRadio_'+refreshSummary.id).setVisibilityMode(Ext.Element.DISPLAY).hide();
