...
Ext.define('SailPoint.LCM.RequestEntitlementsDetailGroup', {
    ...
    showSelectAllToggled: function(checkbox, checked) {
        var selectAllDiv = Ext.get('selectAllDiv_' + this.identityGroup.id);
        var idsDiv = Ext.get('entitlementIdentitiesDiv_' + this.identityGroup.id);
    ...
    validateCombo: function(combo, isInstance, id) {
        ...
        if (combo) {
            ...
            var errorDiv = Ext.get('error_' + typeString + '_' + id);
            if (!combo.getValue()) {
                valid = false;
                errorDiv.show();
            }
    ...
    submit: function() {
        if (this.selectAllCheckbox && this.selectAllCheckbox.checked) {
            ...
            for (var i=0; i<this.identityGroup.entitlementIdentities.length; i++) {
                ...
                Ext.getDom('editForm:hidden_instance_' + id).value = instance;
                Ext.getDom('editForm:hidden_nativeIdentity_' + id).value = nativeIdentity;
                Ext.getDom('editForm:hidden_createAccount_' + id).value = createAccount;
                Ext.getDom('editForm:hidden_forceCreateAccount_' + id).value = forceCreate;
            }
        }
        else {
            for (var i=0; i<this.getInstanceCombos().length; i++) {
                ...
                Ext.getDom('editForm:hidden_instance_' + combo.entitlementIdentityId).value = instance;
            }
            for (var i=0; i<this.getNativeIdentityCombos().length; i++) {
                ...
                Ext.getDom('editForm:hidden_nativeIdentity_' + combo.entitlementIdentityId).value = nativeIdentity;
                Ext.getDom('editForm:hidden_createAccount_' + combo.entitlementIdentityId).value = createAccount;
                Ext.getDom('editForm:hidden_forceCreateAccount_' + combo.entitlementIdentityId).value = forceCreate;
            }
        }
    }
});
