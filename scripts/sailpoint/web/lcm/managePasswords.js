...
SailPoint.LCM.ManagePasswords.initializePage = function(gridMetaData, currentIdentity, currentRequests, selfService, generate) {
    ...
    SailPoint.LCM.ManagePasswords.accountsGrid = new SailPoint.grid.PagingCheckboxGrid({
        ...
        width: Ext.getDom('managePasswordsContainer').clientWidth,
...
SailPoint.LCM.ManagePasswords.renderCurrentRequests = function() {
    ...
    if (store.getTotalCount() == 0) {
        ...
        var selectMethodContainer = Ext.get('passwordSelectMethodContainer');
        if (selectMethodContainer) {
          selectMethodContainer.hide();
        }
        Ext.getCmp('managePasswordsGrid').hide();
        var mesgDiv = Ext.getDom('noPasswordsContainer');
        var innerMsg = '#{msgs.lcm_manage_passwords_err_no_accounts}';
        mesgDiv.innerHTML = innerMsg;
        Ext.get('submitBtn').setVisibilityMode(Ext.Element.DISPLAY).hide();
        return;
    }
                ...
                if (!isGenerate && !isSync || SailPoint.LCM.ManagePasswords.selfService) {
                    ...
                    if (requests) {
                        ...
                        if (SailPoint.LCM.ManagePasswords.selfService && record.get('supports_current_password')) {
                          ...
                          if (currentF) {
                              Ext.get(currentF.parentNode.parentNode).setVisibilityMode(Ext.Element.DISPLAY).hide();
                          }
                        }
    ...
    var accIds = Ext.getDom('editForm:accountIds').value;
...
SailPoint.LCM.ManagePasswords.isSynchronize = function() {
    return Ext.getDom('editForm:syncCheckbox').checked;
};
...
SailPoint.LCM.ManagePasswords.generatePasswordsSelected = function() {
    ...
    Ext.getDom('editForm:generate').value = getSelectedRadioInput('generateRadio');
...
SailPoint.LCM.ManagePasswords.resetSyncOptions = function() {
    Ext.getDom('editForm:syncCheckbox').checked = false;
    Ext.getDom('editForm:syncPassword').value = '';
    Ext.getDom('editForm:syncConfirm').value = '';
};
...
SailPoint.LCM.ManagePasswords.optionsChanged = function() {
    ...
    Ext.get('synchronizePasswords')[(isGenerate) ? 'hide' : 'show']();

    // Only show the sync fields if sync'ing.
    Ext.get('syncFields')[(isSync) ? 'show' : 'hide']();
    ...
    if(!SailPoint.LCM.ManagePasswords.selfService) {
      Ext.get('generateFields')[(isGenerate) ? 'show' : 'hide']();
    }
...
SailPoint.LCM.ManagePasswords.copyAccountSelections = function() {
    ...
    if(!selModel.isAllSelected() && selModel.getSelectedIds().length < 1) {
        ...
        Ext.getDom('editForm:accountIds').value = arrayToString(selections, false);
        Ext.getDom('editForm:selectAllAccounts').value = selModel.isAllSelected();
        return true;
    }
};
...
SailPoint.LCM.ManagePasswords.validate = function() {
    ...
    if (!SailPoint.LCM.ManagePasswords.isGenerate() &&
        ...
        var syncPass = Ext.getDom('editForm:syncPassword').value;
        var syncConfirm = Ext.getDom('editForm:syncConfirm').value;
        ...
        if (SailPoint.LCM.ManagePasswords.selfService) {
            ...
            Ext.getDom('editForm:currentPwds').value = hashToMapString(cpwdsArr);
        }
    ...
    if (errors.length == 0) {
        ...
        errorDiv.innerHTML = '';
        Ext.getDom('editForm:validateAjBtn').click();
        return true;
    }
    else {
        var errorTable = "<ul>";
        ...
        if (errorDiv.style.display == 'none') {
            Ext.get('editForm:serverErrors').setVisibilityMode(Ext.Element.DISPLAY).hide();
            Ext.get('errorMessages').show();
        }
        return false;
    }
};
...
SailPoint.LCM.ManagePasswords.getGridRequests = function() {
    ...
    var requests = [];
    Ext.Object.each(changedPasswordsById, function(key, val) {
        var record = SailPoint.LCM.ManagePasswords.accountsGrid.getStore().getById(key);
        var request = SailPoint.LCM.ManagePasswords.createAccountRequestFromRecord(key, record, val);
...
SailPoint.LCM.ManagePasswords.linkPasswordPolicyRenderer = function(value, p, r) {
  ...
  return Ext.String.format('{0} <img class="passwordHlp" id="imgHlp_{1}_{4}" style="padding-left:10px" width="16" height="16" src="{2}" alt="{3}" title=""/>', value, value, src, rst, generateTabId());
};
