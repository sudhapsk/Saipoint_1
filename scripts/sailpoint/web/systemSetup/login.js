...
SailPoint.SystemSetup.Login = {
    ...
    initializePage: function(activeTab, isLcmEnabled) {
        ...
        var tabPanel = new Ext.TabPanel({
            ...
            width: Ext.getDom('loginTabs').clientWidth,
            items: tabItems
        });
    },
    ...
    showTabPane: function(id, index) {
        ...
        Ext.getDom('configForm:activeTab').value = index;
    },
    ...
    persist: function() {
        var identityAttExt = Ext.getCmp(SailPoint.SystemSetup.Login.phoneAttributeId), 
            phoneAttHidden = Ext.getDom('configForm:phoneAttributeHidden');
    ...
    handleValidate: function() {
        var msgs = Ext.decode(Ext.getDom('configForm:msgs').value);
        ...
        if ((warnings.length > 0) && (errors.length === 0) && !hasPageErrors) {
            ...
            Ext.MessageBox.show({
                ...
                fn: function(btnId) {
                        if ('ok' === btnId) {
                            Ext.getDom('configForm:stealthSaveButton').click();
                        }
                    }
            });
        }
    ...
    getLoginPanelHeight: function() {
        var lockoutCheckbox = Ext.getDom('configForm:enablelockout');
    ...
    doQuestionAction: function(questionId, btnName) {
        Ext.getDom('configForm:selectedQuestionId').value = questionId;
        Ext.getDom('configForm:' + btnName).click();
        SailPoint.SystemSetup.Login.makePageDirty();
    },

    enableOrDisableLockoutDuration: function() {
        var maxFailsVal = Ext.getDom('configForm:maxAuthQuestionFailures').value;
        var maxFails = parseInt(maxFailsVal);
        var disable = (!isNaN(maxFails) && (maxFails < 1));
        Ext.getDom('configForm:authQuestionLockoutDurationMinutes').disabled = disable;
    },
