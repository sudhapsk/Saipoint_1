 ...
 */
SailPoint.Workflow = function(wfInfo) {
 ...
 * @param  wfInfo  The raw JSON workflow mapping information object.
 */
SailPoint.WorkflowVariable = function(variableInfo) {
    if (variableInfo) {
        ...
        this.initializerNegate = variableInfo.initializerNegate;
 ...
 * @param  wfInfo  The raw JSON workflow mapping information object.
 */
SailPoint.WorkflowStep = function(stepInfo) {
    this.transitions = [];
    this.args = [];
    this.returns = [];
    ...
    if (stepInfo) {
        ...
        if (stepInfo.replicator) {
            this.replicator = new SailPoint.WorkflowStepReplicator(stepInfo.replicator);
        }
 ...
 * @param  wfInfo  The raw JSON workflow mapping information object.
 */
SailPoint.WorkflowStepTransition = function(transitionInfo) {
    if (transitionInfo) {
 ...
 * Constructor.
 *
 * @param  approvalInfo  The raw JSON workflow mapping information object.
 */
SailPoint.WorkflowApproval = function(approvalInfo) {
    this.children = [];
    this.args = [];
 ...
 * @param  wfInfo  The raw JSON workflow mapping information object.
 */
SailPoint.WorkflowArgument = function(argumentInfo) {
    if (argumentInfo) {
        ...
        this.negate = argumentInfo.negate;
    }
};
 ...
 * @param  wfInfo  The raw JSON workflow mapping information object.
 */
SailPoint.WorkflowWorkItemConfig = function(wiConfigInfo) {
 ...
 * @param  wfInfo  The raw JSON workflow mapping information object.
 */
SailPoint.WorkflowForm = function(formConfig) {
    if (formConfig) {
        ...
        // Extended attributes
        this.attributes = formConfig.attributes;

        // Set the form items root. The root is
        // the tree structured form items object.
        // It is used to initialize the tree store.
        this.items = formConfig.root;
    }
};
 ...
 */
SailPoint.RuleLibrary = function(libInfo) {
    if (libInfo) {
        this.id = libInfo.id;
 ...
 */
SailPoint.WorkflowStepReturn = function(returnInfo) {
    if (returnInfo) {
...
////////////////////////////////////////////////////////////////////////////////
//
// A javascript representation of a SailPoint.object.Workflow.Replicator
// object
//
////////////////////////////////////////////////////////////////////////////////
SailPoint.WorkflowStepReplicator = function(replicator) {
    if (replicator) {
        this.itemsVar = replicator.itemsVar;
        this.arg = replicator.arg;
    }
};

SailPoint.WorkflowStepReplicator.prototype = {
    itemsVar: null,
    arg: null
};
