...
Ext.define('SailPoint.WorkflowStepPanel', {    
    ...
    editForm : function(stepId) {
        ...
        this.formWindow = new SailPoint.form.editor.WorkflowFormEditorWindow({
            beanType: '#{msgs.workflow_form}',
            panel: this
        });
        ...
        this.formWindow.formEditor.setStep(stepId, step);
    ...
    save : function () {
        ...
        for(s = 0; s < steps.length; s++) {
            ...
            if ((!Ext.isDefined(wfStep.form.sections)
                    || !Ext.isDefined(wfStep.form.buttons))
                        && Ext.isDefined(wfStep.form.items)) {
                this.transformFormItemsTreeToArray(wfStep.form);
            }

            // Discard the form.items property. The form.item
            // is tree structured object, therein it is not
            // needed in save operation.
            delete wfStep.form.items;

            wfStep.saveTransitions(steps);
        }
    },

    /**
     * Converts form items tree to flat section array and button array.
     */
    transformFormItemsTreeToArray : function (form) {
        var formEditor,
            cloneOfItems,
            itemsTree = form.items; // tree structured form items

        // Initialize dummy FormEditor class.
        // Do not try to load the UI of form editor.
        formEditor = new SailPoint.form.editor.FormEditor({
        ...
        cloneOfItems = JSON.parse(JSON.stringify(itemsTree));

        // Initialize tree store
        formEditor.formItemStore.setRootNode(cloneOfItems);

        // Flatten out the form items tree into sectionArray and buttonArray
        formEditor.flattenOutFormItemsTree(formEditor.formItemStore.getRootNode());

        form.sections = formEditor.sectionArray;
        form.buttons = formEditor.buttonArray;
    },
