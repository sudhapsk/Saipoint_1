...
Ext.define('SailPoint.WorkflowEditor', {
                          ...
                          Ext.MessageBox.show({
                              ...
                              fn: function(btn) {  
                                  if(btn=='cancel') {
                                      ...
                                      Ext.getDom("editForm:workflowId").value = this.panel.workflow.workflowId;
                                      Ext.getDom("editForm:deleteWorkflowBtn").click();
                                      this.disable();
                                      return null;
                                  }
                              },
    ...
    updateConfigForm : function(configFormName, configFormContext, configFormStepName, parentComponentId, currentArgJSON, contextId) {
        var formName = Ext.getDom('updateConfigForm:configFormName');
        formName.value = configFormName;
        var formContext = Ext.getDom('updateConfigForm:configFormContext');
        formContext.value = configFormContext;
        var stepName = Ext.getDom('updateConfigForm:configFormStepName');
        stepName.value = (!Ext.isEmpty(configFormStepName)) ? configFormStepName : '';
        
        var compId = Ext.getDom('updateConfigForm:parentComponentId');
        compId.value = parentComponentId;
        
        var argJSON = Ext.getDom('updateConfigForm:argJSON');
        argJSON.value = currentArgJSON;
        
        var ctxId = Ext.getDom('updateConfigForm:contextId');
        ctxId.value = contextId;
        
        var updateButton = Ext.getDom('updateConfigForm:updateConfigFormButton');
        updateButton.click();
        
    },
    
    loadConfigForm: function(config) {
        var context = Ext.getDom('updateConfigForm:configFormContext');
...
Ext.define('SailPoint.WorkflowVariableAdvancedPanel', {
    ...
    addVariable : function (variable, deferInsert) {
      ...
      if(variable) {
        ...
        if(variable.initializerSource) {
          ...
          initializerNegate = variable.initializerNegate;
        }
      } else {
      ...
      var scriptRadio = {
          ...
          width:700,
          runInitialLoad : [initializerMethod, initializerSource, initializerNegate],
          variableName: vName,
          useNegate: true
      };
      ...
      var ctr = {
          ...
          items: [
            ...
            {xtype:'textarea', fieldLabel:'#{msgs.label_description}',name: 'description', itemId: 'descr', width:600, value: Ext.String.htmlDecode(vDescription), itemCls: 'blocklabel', style:'margin-bottom:20px',
                listeners: {
                    blur: function(field, opts){
                        var uppan = this.findParentByType('fieldset');
    ...
    getArgsJSON : function() {
        ...
        this.items.each(function(f) {
            ...
            var initializerNegate = scriptRadio.getNegate();
            ...
            if (!Ext.isEmpty(initializerSource)) {
                ...
                variable.initializerNegate = initializerNegate;
    ...
    save : function() {
        ...
        this.items.each(function(f) {
          ...
          var variable = this.parent.editor.workflow.getVariableById(varId);
          if(variable) {
            variable.name = varName;
            variable.input = varInput;
            variable.editable = varEditable;
    ...
    load : function (variables) {
        if (this.loaded === false) {
            ...
            for(var vIndex=0; vIndex<vCount; vIndex++) {
                varArray.push(this.addVariable(variables[vIndex], true));
            }
            this.add(varArray);
            this.loaded = true;
        }
...
Ext.define('SailPoint.WorkflowMetricsPanel', {
    ...
    updateTimes: function(timeUnits) {
        ...
        if (this.avgTimeInMinutes > 0) {
            ...
            avgTime = new Number(Ext.getDom('workflowMetricsAvgTime').innerHTML);
        ...
        if (this.maxTimeInMinutes > 0) {
            ...
            maxTime = new Number(Ext.getDom('workflowMetricsMaxTime').innerHTML);
            this.maxTimeInMinutes = maxTime;
        }
        
        avgTime = this.updateTimeToUnits(avgTime, timeUnits);
        Ext.getDom('workflowMetricsAvgTime').innerHTML = avgTime;
        maxTime = this.updateTimeToUnits(maxTime, timeUnits);
        Ext.getDom('workflowMetricsMaxTime').innerHTML = maxTime;
