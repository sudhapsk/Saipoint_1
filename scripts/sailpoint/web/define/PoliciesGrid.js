...
Ext.define('SailPoint.Define.Grid.Policy', {
    statics : {
        ...
        clickRow : function(gridView, record, HTMLitem, index, e, eOpts) {
            Ext.getDom('policyForm:currentObjectId').value = record.getId();
            Ext.getDom('policyForm:editButton').click();
        },

        edit : function() {
            // there's no functional difference btwn this and clickApplication
            Ext.getDom('policyForm:currentObjectId').value = id;
            Ext.getDom('policyForm:editButton').click();
        },
        ...
        deletePolicy : function(button, text) {
            if (button == 'yes') {
                Ext.getDom('policyForm:currentObjectId').value = id;
                Ext.getDom('policyForm:deleteButton').click();
            }
        },

        newPolicy : function(sel) {
            Ext.getDom('policyForm:newPolicyType').value = sel.value;
            Ext.getDom('policyForm:createPolicyButton').click();
        }
    }
// end statics
});
