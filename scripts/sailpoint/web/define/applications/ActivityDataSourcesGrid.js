...
SailPoint.activityDS.initDSContent = function(){
  ...
  if (!SailPoint.activityDS.isLoaded) {
        Ext.QuickTips.init();
        var gridWidth = Ext.getDom('activityDataSources-display').clientWidth;
        if (gridWidth == 0)
            gridWidth = Ext.getDom('activitiesTbl').clientWidth;
...
SailPoint.activityDS.showContextMenu = function(gridView, record, HTMLitem, index, e, eOpts){
    ...
    var canEdit = Ext.getDom("activityDSCanEdit").value;
...
SailPoint.activityDS.clickRow = function(gridView, record, HTMLitem, index, e, eOpts){
    Ext.getDom('editForm:selectedDSForApp').value = record.getId();
...
SailPoint.activityDS.editADS = function(menuItem, eventObj) {
    ...
    Ext.getDom('editForm:selectedDSForApp').value = id;
    Ext.getDom('editForm:editButton').click();
};
