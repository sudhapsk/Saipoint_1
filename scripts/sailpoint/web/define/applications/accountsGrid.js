...
Ext.define('SailPoint.Define.Applications.AccountsGrid', {
    statics: {
        getAccountsGridConfig: function() {
            var gridMetaData = Ext.decode(Ext.getDom('applicationAccountGridMetadata').innerHTML);
            ...
            var gridConfig = {
                ...
                url : SailPoint.getRelativeUrl('/rest/applications/' + makeEscapedQueryParam(Ext.getDom('editForm:id').value) + '/links/accountsGrid'),
        ...
        renderIdentityNavLink: function(value, metadata, record, rowIndex, colIndex, store) {
            var hasViewIdentityRight = (Ext.getDom('hasIdentityViewRight').innerHTML.toLowerCase() == "true");
            var renderedValue;
            
            if (hasViewIdentityRight) {
                metadata.tdAttr = 'onclick="Ext.EventManager.stopEvent(this); ' +
                                           'Ext.getDom(\'viewIdentityForm:identityToView\').value = \'' + record.get('identity-id') + '\';' +
                                           'Ext.getDom(\'viewIdentityForm:currentTab\').value = \'accountsGridContent\'; ' +
                                           'Ext.getDom(\'viewIdentityForm:viewIdentityBtn\').click();"';
