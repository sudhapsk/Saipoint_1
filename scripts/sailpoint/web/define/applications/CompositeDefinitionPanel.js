            ...
            if (!this.correlationMappingGrid){
                ...
                if ( !this.expressionPanel ) {
                    ...
                    saveSelectorButton.on('click', function(button, event) {
                        // When save is clicked, persist both the selector for attribute matching
                        ...
                        Ext.getDom("editForm:saveSelector").click();
                        saveSelectorButton.hide();
                        // mark the config dirty so we can save it if necessary
                        Ext.getDom("editForm:logicalConfigDirty").value = 'true';
                    }, saveSelectorButton);
    ...
    */
        ...
        appGrid.on('cellclick', function(gridView, td, cellIndex, r, tr, rowIndex, e, eOpts) {
            this.getStore().each(function(record) {
                ...
                if (idx == rowIndex) {
                    ...
                    Ext.getDom('editForm:currentTierApp').value = record.get('application');
                } 
                else {
        ...
        removeButton.on('click', function(button, event) {
            ...
            if (selectedIds) {
                ...
                if ( length > 0 ) {
                    ...
                    Ext.getDom('editForm:currentTierApp').value = null;
                    Ext.getDom('editForm:switchTiers').click();
                }
                ApplicationPage.persistCompositeDefinition();
            }
            this.deselectAll();
    ...
    initCorrelationGrid : function() {
        ...
        grid.on('tierSelectionChanged', function(newTierRecord) {
            ...
            Ext.getDom('editForm:currentTierApp').value = newTierRecord.get('application');
            Ext.getDom('editForm:switchTiers').click();
            ...
            if(tier) {
                tier.getStore().getProxy().extraParams = {objType:'account', name: Ext.getDom('editForm:currentTierApp').value };
                tier.getStore().load();
            }
        });
