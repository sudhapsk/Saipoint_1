...
SailPoint.roles.getMiningTemplatesPanel = function(config) {
  ...
  var templatesColumnConfig = Ext.JSON.decode(Ext.getDom('miningTemplatesGridConfig').value);
...
SailPoint.roles.ExecuteMiningTemplate = function(id, type, name) {
    if( type == 'IT Role Mining' ) {
	    ...
	    Ext.getDom('templateId').value = id;
...
SailPoint.roles.DeleteMiningTemplate = function(btn, text) {
  if(btn=='yes') {
    Ext.getDom('tabState:miningSelectedId').value = MINING_TASK_ID;
    Ext.getDom('tabState:miningDeleteBtn').click();
  }
};
