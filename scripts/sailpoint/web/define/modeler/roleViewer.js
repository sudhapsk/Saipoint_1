...
SailPoint.Role.Viewer.getRoleViewerPanel = function(config) {
    ...
    var errors = Ext.getDom('spErrorMsgsDiv').innerHTML;
...
SailPoint.Role.Viewer.getNavPanelWrapper = function(config) {
    var navPanelWrapper = {
        ...
        bbar: [{
            ...
            handler: function() {
                Ext.getDom('viewerForm:roleToEdit').value = '';
    ...
    Ext.MessageBox.confirm(
        ...
        function(button) {
            if (button == 'yes' && viewerState.selectedRoleId !== null && viewerState.selectedRoleId != '') {
                Ext.getDom('viewerForm:roleToEdit').value = viewerState.selectedRoleId;
                ...
                if (roleTreeFilter.view && roleTreeFilter.view.getSelectedRecords()[0].getId() == Ext.getDom('viewerForm:roleToEdit').value)
                    roleTreeFilter.setValue('');

                Ext.getDom('viewerForm:deleteRole').click();
            }
        }
    );    
};
    ...
    Ext.MessageBox.confirm(
        ...
        function(button) {
            if (button == 'yes' && currentNodeObjId) {
                Ext.getDom('viewerForm:roleToEdit').value = currentNodeObjId;
...
Ext.define('SailPoint.Role.Viewer.NodeMenu', {
    ...
    items: [{
        ...
        handler: function(item, eventObj) {
            ...
            var currentNodeObjId = nodeObjIds[nodeObjIds.length - 1];
            Ext.getDom('viewerForm:roleToEdit').value = '';
            Ext.getDom('viewerForm:roleToAddTo').value = currentNodeObjId;
            // Hack to prevent a stupid script error caused by JSF
            Ext.getDom('viewerForm:addToRole').onclick = function(){};
            Ext.getDom('viewerForm:addToRole').click();
        }
    }, {
        id: 'cloneRoleOption',
        ...
        handler: function(item, eventObj) {
            ...
            var currentNodeObjId = nodeObjIds[nodeObjIds.length - 1];
            Ext.getDom('viewerForm:roleToEdit').value = '';
            Ext.getDom('viewerForm:roleToClone').value = currentNodeObjId;
            // Hack to prevent a stupid script error caused by JSF
            Ext.getDom('viewerForm:cloneRole').onclick = function(){};
            Ext.getDom('viewerForm:cloneRole').click();
        }
    }, {
...
SailPoint.Role.Viewer.displayMenu = function(record, point) {
    if (record.get('editable') && Ext.getDom('roleViewerRight').innerHTML == 'ManageRole') {
...
SailPoint.Role.Viewer.initViewer = function () {
    ...
    Ext.getDom('viewerForm:editRole').onclick = function () {
...
SailPoint.Role.Viewer.initNavPanel = function() {
    ...
    navPanel.canEditRole = Ext.getDom('roleViewerRight').innerHTML == 'ManageRole';
...
SailPoint.Role.Viewer.showDeletionResults = function() {
    var deleteSucceeded = Ext.DomQuery.select('li[class=formError]', Ext.getDom('viewerMsgs')).length === 0;
    ...
    SailPoint.TabPanel.applyMessage(viewerPanel, Ext.getDom('spErrorMsgsDiv'));
};
