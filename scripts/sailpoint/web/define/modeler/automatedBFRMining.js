...
Ext.define('SailPoint.roles.AutomatedBFRMiningPanel', {
    ...
    displayLaunchStatus: function() {
        var launchResults = Ext.getDom('businessRoleLaunchResultContents').innerHTML;
    ...
    displaySaveStatus: function() {
        var saveResults = Ext.getDom('businessRoleSaveResultContents').innerHTML;
      ...
      this.getLoader().load({
          ...
          callback: function() {
              var bfrOwnerSuggest = new SailPoint.IdentitySuggest({
                  ...
                  renderTo: Ext.getDom('bfrMiningOwnerSuggest'),
                  binding: Ext.getDom('bfrMiningForm:bfrMiningOwner'),
                  allowBlank: false,
                  rawValue: Ext.getDom('bfrMiningOwnerSuggestValue').value,
                  emptyText: '#{msgs.select_owner}',
                  baseParams: {context: 'Owner'}
              });

        	  var ownerValue = Ext.getDom('bfrMiningForm:bfrMiningOwner').value;
              if ('' != ownerValue) {
            	  var options = {
            	      params: {
            	    	  query: Ext.getDom('bfrMiningOwnerSuggestValue').value
            	      },
            	      callback: function() {
              ...
              var bfrScopingSuggest = new SailPoint.MultiSuggest({
                  ...
                  jsonData: Ext.decode(Ext.getDom('bfrMiningScopingAttributesMultiSuggestData').innerHTML),
              ...
              var bfrContainerSuggest = new SailPoint.BaseSuggest({
                  ...
                  renderTo: Ext.getDom('bfrContainerRoleSuggest'),
                  binding: Ext.getDom('bfrMiningForm:bfrContainerRole'),
                  allowBlank: false,
                  emptyText: '#{msgs.select_container_role}'
              });

              var containerRoleValue = Ext.getDom('bfrMiningForm:bfrContainerRole').value;
              if ('' != containerRoleValue) {
            	  var options = {
            	      params: {
            	    	  query: Ext.getDom('bfrContainerRoleSuggestValue').value
            	      },
            	      callback: function() {
              ...
              var bfrMiningSuggest = new SailPoint.MultiSuggest({
                  ...
                  jsonData: Ext.decode(Ext.getDom('bfrMiningApplicationMultiSuggestData').innerHTML),
              ...
              buildTooltips(Ext.getDom('bfrMining'));
              Ext.MessageBox.hide();
              SailPoint.roles.setITRoleOptionsDisabled(Ext.getDom('bfrMiningForm:bfrAttachProfilesToBfr').checked);
              SailPoint.roles.setEntitlementOptionsDisabled(!Ext.getDom('bfrMiningForm:bfrMineForEntitlements').checked);
              this.updateContainerOption();
              
              if( Ext.getDom( 'bfrMiningForm:bfrRoleMiningTemplateId' ).value == '' ) {
    ...
    updateContainerOption: function() {
        var roleTypeOption = Ext.getDom('orgRoleTypeOption');
        roleTypeOption.style['display'] = (Ext.getDom('bfrMiningForm:generateOrgRoles').checked && !Ext.getDom('bfrMiningForm:bfrAnalyzeOnly').checked) ? '' : 'none';
        var existingRootContainerOption = Ext.getDom('existingRootContainer');
        existingRootContainerOption.style['display'] = Ext.getDom('bfrMiningForm:generateOrgRoles').checked ? 'none' : '';
    },
    
    toggleRoleCreationOptions: function() {
        var roleCreationOptions = Ext.DomQuery.select('tr[class*=roleCreationOption]', 'bfrMining');
        var isRoleCreationEnabled = !Ext.getDom('bfrMiningForm:bfrAnalyzeOnly').checked;
    ...
    if (!businessRoleMiningPanel) {
        businessRoleMiningPanel = new SailPoint.roles.AutomatedBFRMiningPanel({
            ...
            bbar: [{
                ...
                handler: function() {
                    ...
                    if(validated) {
                        SailPoint.roles.saveTemplatePrompt({
                            ...
                            templateName: Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateName').value,
                            originalTemplateName: Ext.getDom('bfrMiningForm:bfrRoleMiningOriginalTemplateName').value,
                            saveFromExistingHandler: function() {
                                if(Ext.getCmp('bfrWindowTemplateSaveNew').checked) {
                                    Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateId').value = '';
                                    Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateName').value = Ext.getCmp('bfrWindowTemplateName').getValue();
                                }
                                Ext.getCmp('bfrMiningTemplateSaveWindow').hide();
                                Ext.getDom('bfrMiningForm:bfrSaveTemplateBtn').click();
                            }, 
                            saveFromScratchHandler: function(btn, text) {
                                Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateId').value = '';
                                Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateName').value = text;
                                Ext.getDom('bfrMiningForm:bfrSaveTemplateBtn').click();
                            }
                        });
                  } else {
                        ...
                        SailPoint.roles.saveTemplatePrompt({
                            ...
                            saveFromExistingHandler: function() {
                                if(Ext.getCmp('bfrSaveAndExecWindowTemplateSaveNew').checked) {
                                    Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateId').value = '';
                                    Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateName').value = Ext.getCmp('bfrSaveAndExecWindowTemplateName').getValue();
                                }
                                Ext.getCmp('bfrSaveAndExecMiningTemplateSaveWindow').hide();
                                Ext.getDom('bfrMiningForm:bfrSaveAndExecTemplateBtn').click();
                            },
                            saveFromScratchHandler: function(btn, text) {
                                Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateId').value = '';
                                Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateName').value = text;
                ...
                handler: function() {
                    Ext.getDom('bfrMiningForm:bfrViewLastTaskResult').click();
                }
            },{
    ...
    if(panel.templateId) {
        if(!saveWindow) {
            ...
            updateField.on('check', function(checkbox) {
                if (checkbox.checked) {
                    Ext.get(type + 'WindowTemplateNameLabel').setVisibilityMode(Ext.Element.DISPLAY).hide();
                    Ext.get(type + 'WindowTemplateName').setVisibilityMode(Ext.Element.DISPLAY).hide();
                }
            });
            ...
            Ext.getDom(type + 'WindowTemplateSaveUpdate').nextSibling.innerHTML = Ext.String.format('#{msgs.role_mining_template_update}', options.originalTemplateName);
        }
...
SailPoint.roles.setITRoleOptionsDisabled = function(isDisabled) {
    ...
    var isRoleCreationDisabled = Ext.getDom('bfrMiningForm:bfrAnalyzeOnly').checked;
    var option;
...
SailPoint.roles.validateBFRSelections = function() {
    ...
    // Check for name
    var nameField = Ext.getDom('bfrMiningForm:bfrRoleMiningTemplateName');
    ...
    var isMiningForITEntitlements = Ext.getDom('bfrMiningForm:bfrMineForEntitlements').checked;
    if (isMiningForITEntitlements) {
        ...
        var percentageThreshold = Number(Ext.getDom('bfrMiningForm:bfrThreshold').value);
    ...
    // 
    var minNumUsers = Ext.getDom('bfrMiningForm:bfrMinNumUsers').value;
