...
Ext.define('SailPoint.template.TemplateFieldEditor', {
  ...
  initComponent : function() {
    ...
    var ownerItems = [
      {boxLabel: '#{msgs.none}', name: 'ownerType', inputValue: SailPoint.template.TemplateEditorConstants.TYPE_NONE, checked: true },
      {boxLabel: '#{msgs.template_editor_owner_requester}', name: 'ownerType', inputValue: SailPoint.template.TemplateEditorConstants.TYPE_REQUESTER},
    ...
    if(Ext.get('editForm:appName')) {
        
        var appName = Ext.get('editForm:appName').getValue();
...
//              onShow: function(){this.bodyEl.up('.x-form-item').setStyle('display','block');}
  ...
  newField : function() {
    ...
    if(Ext.getCmp('fieldName').getXType() !== 'textfield') {
        ...
        fieldNameStore.loadData(Ext.JSON.decode(Ext.getDom('createAttributes').value));
      }
  ...
  validateDuplicates : function() {
    ...
    if(name) {
      ...
      /** Check against other forms with same application **/
      var templatesField = Ext.getDom('formsJSON');
      if(templatesField) {
        var templates = Ext.JSON.decode(templatesField.value);
        if(templates.length>1 && Ext.getDom('editForm:formId')) {
          var templateId = Ext.getDom('editForm:formId').value;
          var app = this.editor.applicationSuggest.getValue();
          for(var i=0; i<templates.length; i++) {
            ...
            /** If this is a different form, but with the same application, check for dupes **/
            if(template.id!=templateId && app == template.application && template.fields.length>0) {
