...
//        }));
        ...
        /* when we hide columns in the grid, they are set to 0 width and no class is added to them
        This sucks since we need to add a class to turn off the white-space: nowrap property that
        will cause the grid rows to grow huge.  This function adds/removes the hidden-cell class.
         */
        function decorateHiddenColumns(column, grid, doHide) {
        ...
        /* Adds the 'hidden-cell' classes to the cells in this column */
        this.on('columnhide', function(ct, column) {
            decorateHiddenColumns(column, this, true);
        }, this);
        /* Remove the 'hidden-cell' class from the cells in the column when it is shown */
        this.on('columnshow', function(ct, column) {
            decorateHiddenColumns(column, this, false);
        }, this);

        this.on('afterrender', function(grid) {
            Ext.each(grid.columns, function(column) {
                if(column.hidden) {
                    /* have to delay it by 100 ms to allow the render to finish */
                    setTimeout(function() {
                        decorateHiddenColumns(column, grid, true);
                    }, 100);
                }
            })
        })
