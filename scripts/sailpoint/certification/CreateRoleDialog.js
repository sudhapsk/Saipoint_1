...
Ext.define('SailPoint.certification.CreateRoleDialog', {
    ...
    initApproverSuggest: function() {
        ...
        this.approverSuggest = new SailPoint.IdentitySuggest({
            ...
            value: Ext.getDom('approverSuggestInitialValue').innerHTML,
            width: 300,
    ...
    initDescriptionEditor: function(){
        ...
        if (Ext.getDom('roleDescriptionHTML')) {
    ...
    save: function() {
        ...
        if (roleDescriptionCmp) {
            Ext.getDom('roleFromCertificationForm:jfDescriptionsJSON').value = roleDescriptionCmp.getCleanValue();
        }
        try {
            Ext.getDom('roleFromCertificationForm:saveRoleBtn').click();
        } catch (err) {
    ...
    onSaveHandler: function() {
        var validationError =
            Ext.getDom('roleFromCertificationForm:validationErrorDetected').value === 'true';
        if (!validationError) {
            this.destroy();
        } else {
            this.reSizeWindow();
