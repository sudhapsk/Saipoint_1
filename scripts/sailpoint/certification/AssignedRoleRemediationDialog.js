        ...
        if (this.hasRecords){
            this.revokeGrid.getStore().each(function(record){
                ...
                if (this.decision.revokedRoles){
                    selected = this.decision.revokedRoles.indexOf(record.internalId) > -1;
                }
                if (selected){
                    this.revokeGrid.getSelectionModel().select(record, true, false);
                }
            }, this);
        }
        ...
        Ext.Ajax.request({
          ...
          success: function(response){
            ...
            if (respObj.remediationAction == "SendProvisionRequest"){
                ...
                Ext.getDom('assignedRoleProvisioningMessageContent').innerHTML = provMsg;
...
Ext.define('SailPoint.PermitsAndReqsGrid', {
    ...
    //Overriding PagingCheckboxGrid getSelectedIds. This function did not cater for removing Ids in the excluded list.
    //The excluded list is populated if the user does a 'select all' in the UI and then removes individual elements. The original implementation would still
    //send all elements to revoke and not remove the excluded ones.
    getSelectedIds : function() {
        var selectedItems = this.selModel.getSelectedIds();
        if (this.selModel.selected.isAllSelected()) {
                var excludedItems = this.selModel.getExcludedIds();
                for (var i=selectedItems.length - 1; i>=0; i--) {
                    for (var j=0; j<excludedItems.length; j++) {
                        if (selectedItems[i] === excludedItems[j]) {
